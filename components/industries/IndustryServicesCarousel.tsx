...
export interface IndustryServicesCarouselProps {
...
const CARD_GAP = 24;
const DEFAULT_AUTOPLAY_DURATION = 6500;

export function IndustryServicesCarousel({
  services,
  industryName,
  autoplayDuration = DEFAULT_AUTOPLAY_DURATION,
}: IndustryServicesCarouselProps) {
  const cards = services.map((s) => ({
    ...
    description:
      s.metaDescription ||
      `Enterprise Microsoft ${s.name} architecture, tenant integration, and compliance engineering.`,
    image: s.imageUrl || `/images/services/${s.slug}.jpg`,
  ...
  // Performance: pause progress loop when scrolled off-screen
  ...
  // On screens under 480px, card shrinks dynamically so active card is centered without screen cutoff
  ...
  return (
    <section
      ref={viewRef}
      aria-label={`Related services for ${industryName}`}
      className="w-full bg-[#FAF8F5] py-16 sm:py-24 border-y border-mp-border/60 overflow-hidden"
